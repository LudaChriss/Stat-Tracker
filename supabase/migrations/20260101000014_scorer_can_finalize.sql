-- A team scorer can finalize their own team's games.
--
-- Phase 2b's promise is that a scorer scores: they can run a game and write its
-- box score, but they do not own the roster. Row-level security already said
-- exactly that — `games` and `game_lines` writes go through can_score_game,
-- which admits team_manager and team_scorer; `players` and `teams` writes go
-- through can_manage_team, which admits only the manager.
--
-- save_game was the one place that disagreed. It is SECURITY DEFINER, so it
-- bypasses those policies entirely and did its own check with can_manage_team.
-- The result: a scorer could score every play of a game and then be refused at
-- the final whistle with "not allowed to record games for this team". Confirmed
-- against a real database before this fix.
--
-- WHAT THIS DOES NOT CHANGE. Not one line of how a game is written. The insert,
-- the conflict target, the guards, the box-score replacement and the perspective
-- flip are all byte-identical to 20260101000013. The only difference is which
-- people get past the front door, and it is strictly a WIDENING: everyone
-- allowed before is still allowed, on exactly the same path. A manager
-- finalizing a game cannot tell this migration happened.

-- -----------------------------------------------------------------------------
-- Helper: may this user score for this team?
--
-- The team-scoped sibling of can_score_game, which needs a game row and so
-- cannot answer before one exists. Built from the same two primitives rather
-- than a new parallel rule, so there is one definition of "scorer" to keep
-- correct.
-- -----------------------------------------------------------------------------
...
as $$
  select exists (
    ...
    where t.id = p_team_id
      and (
        public.is_team_member(p_team_id, array['team_manager', 'team_scorer'])
        or (t.league_id is not null and public.is_league_admin(t.league_id))
      )
  );
$$;

comment on function public.can_score_team(uuid) is
  'SECURITY DEFINER: true if auth.uid() is team_manager or team_scorer of p_team_id, or league_admin of its league. The team-scoped counterpart of can_score_game.';
...
begin
  ...
  -- CHANGED, and only this: was can_manage_team. A scorer finishes games too.
...
comment on function public.save_game(uuid, jsonb) is
  'Write one finalized game and its box score, idempotently by client id. Refuses a game with no lines, no score, or a result contradicting the score. Open to a team manager, a team scorer, or the league admin.';
