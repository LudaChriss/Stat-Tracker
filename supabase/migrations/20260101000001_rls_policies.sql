...
-- -----------------------------------------------------------------------------
-- Helper: has the current request got *some* authenticated user?
-- (Small readability helper; not security-critical on its own.)
-- -----------------------------------------------------------------------------
...
comment on function public.current_profile_id() is
  'Shorthand for auth.uid(); SECURITY INVOKER, no elevated rights needed.';
